...
import MatchBookMakerItem from "../components/MatchBookMakerItem";

export default class MatchListBookMakerScreen extends React.Component {
  ...
  componentDidMount() {
    this.listenerFocus = this.props.navigation.addListener('focus', () => {
      ...
      let url = `${this.props.baseURL}/typer/matchesBookMakerList?${queryString}`;
      console.log(url);
      fetch(url, {
        method: 'GET',
        headers: {
  ...
  createMatchList() {
    ...
    for (let i in this.state.matchList) {
      if (this.state.matchList[i].competition.id === this.state.competitionList[this.state.competitionIndex].id) {
        number++;
        matchList.push(<MatchBookMakerItem
          key={i}
          number={number}
          ...
          betOutcome={this.state.matchList[i].betOutcome}
          outcomeFullTime={this.state.matchList[i].outcomeFullTime}
          ...
          pointsWon={this.state.matchList[i].pointsWon}
        />)
      }
    }
    return matchList;
